...
namespace OtpGenerator.CommandLine.Logic
{
    public class CommandLineParser : ICommandLineParser
    {
        ...
        /// </summary>
        /// <param name="includeOperations"></param>
        /// <param name="includeContextCommands"></param>
        /// <returns></returns>
        public IList<CommandLineOptionType> GetSpecifiedOptions(bool includeOperations, bool includeContextCommands)
        {
            List<CommandLineOptionType> results = [];

            if (includeOperations)
            {
                results.AddRange(SpecifiedOperations());
            }

            if (includeContextCommands)
            {
                results.AddRange(SpecifiedContextOptions());
            }
        ...
        /// <summary>
        /// Return a list of operations specified on the command line
        /// </summary>
        /// <returns></returns>
        private IList<CommandLineOptionType> SpecifiedOperations()
            => [.. _options.Where(x => x.IsOperation && _values.ContainsKey(x.OptionType)).Select(x => x.OptionType)];

        /// <summary>
        /// Return a list of context manipulation options specified on the command line
        /// </summary>
        /// <returns></returns>
        private IList<CommandLineOptionType> SpecifiedContextOptions()
            => [.. _options.Where(x => !x.IsOperation && _values.ContainsKey(x.OptionType)).Select(x => x.OptionType)];
    }
}
